...
#include "grl/ros/RosArmDriver.hpp"

// original code used floats due to v-rep
// ros uses doubles internally
...
///
class KukaRosPlugin : public std::enable_shared_from_this<KukaRosPlugin> {
public:
...
void initHandles() {
    ...
    RosArmDriverP_ = std::make_shared<RosArmDriver>
    (
        std::make_tuple(
...
bool getStateFromRos(){
            if(!allHandlesSet || !RosArmDriverP_) return false;
			
            RosArmDriver::State armState;
    
            bool isError = RosArmDriverP_->getState(armState);
    
            if(isError) return isError;
    
            simJointPosition             = std::get<RosArmDriver::JointPosition>      (armState);
            simJointForce                = std::get<RosArmDriver::JointForce>         (armState);
            simJointTargetPosition       = std::get<RosArmDriver::JointTargetPosition>(armState);
            simJointTransformationMatrix = std::get<RosArmDriver::JointMatrix>        (armState);
